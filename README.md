...
```dax
Total Restaurant = DISTINCTCOUNT('Your_Table_Name'[Restaurant_ID])

Average Price = AVERAGE('Your_Table_Name'[Price])

Average Rating = AVERAGE('Your_Table_Name'[Rating])

Avg Delivery Time = AVERAGE('Your_Table_Name'[Delivery_Time])
Price Range = SWITCH(TRUE(), 'Your_Table_Name'[Price] <= 200, "Budget(Under 200)", 'Your_Table_Name'[Price] > 200 && 'Your_Table_Name'[Price] <= 500, "Mid Range(201-500)", "Premium(Above 500)")
...
Price Segmentation (Donut Chart)

Monitors marketplace distribution to maintain a balanced portfolio between budget expansion and premium vendors.
